...
public interface KnowledgeDao extends BaseMapper<Knowledge> {
    ...
    @Select("SELECT knowledge.* from knowledge join knowledge_resource " +
            "on knowledge.id_knowledge= knowledge_resource.id_knowledge" +
    ...
    @Select("select knowledge.* from knowledge join knowledge_resource " +
            "on knowledge.id_knowledge= knowledge_resource.id_knowledge " +
            "group by knowledge.id_knowledge order by count(*) desc ")
    IPage<Knowledge> findTop(Page<Knowledge> page);
}
